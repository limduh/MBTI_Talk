...
class SignUpActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.appCompatImageView5.setOnClickListener {
            Log.d("SignIn","#dudu setOnClick appCompatImageView5")
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                if (checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED ||
                    ...
                    Log.d("SignIn","#dudu setOnClick requestPermissions ")
                    requestPermissions(permission, 100)
                }else {
                    Log.d("SignIn","#dudu setOnClick galleryLauncher")
                    galleryLauncher.launch("image/*")
                }
        ...
        binding.SignUpBtnSignUp.setOnClickListener {
            ...
            if (SignupActivity_id.isNotEmpty() && SignupActivity_pass.isNotEmpty() && SignupActivity_confirmPass.isNotEmpty()) {
                if(pattern.matcher(SignupActivity_id).matches()){
                    //이메일 맞음!
                } else {
                    //이메일 아님!
                    Toast.makeText(this, "이메일 형식 아님", Toast.LENGTH_SHORT).show()
                                ...
                                if (selectedUri != null) {
                                    ...
                                    uploadImage(selectedUri) {
                                        if (it != null) {
                                            ...
                                            Toast.makeText(this@SignUpActivity, "이미지 업로드 완료", Toast.LENGTH_SHORT)
                                                .show()

                                            val userId = firebaseAuth.currentUser?.uid
                                            if (userId != null) {
                                                Log.d("SignUp","#dudu+ $user_gender")
                                                val user = UserData(SignupActivity_id, SignupActivity_age, SignupActivity_nickName, userId, user_gender,"",it)//,SignUpActivity_uri )
                                                // DB저장
                                            ...
                                            Toast.makeText(this@SignUpActivity, "이미지 업로드 실패", Toast.LENGTH_SHORT)
                                                .show()
                                        }
                                    }
                                } else {
                                    Toast.makeText(this@SignUpActivity, "이미지를 선택해주세요", Toast.LENGTH_SHORT).show()
                                }
    ...
    override fun onRequestPermissionsResult(
        ...
        Log.d("SignIn","#dudu onRequestPermissionsResult")
        Log.d("SignIn","#dudu requestCode = $requestCode")
        Log.d("SignIn","#dudu permissions = $permissions")
        Log.d("SignIn","#dudu grantResults = ${grantResults[0]}")


        if (grantResults.isEmpty() || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
            when (requestCode) {
                100 -> {
                    Log.d("SignIn","#dudu permissionok = ${grantResults[0]}")
                    Toast.makeText(
                        this@SignUpActivity,
                        "권한 허용됨",
                        Toast.LENGTH_LONG
